...
class AdmCalificacionWindow extends VistaGenericaWindow {
	...
	override opciones(Panel panel) {
		...
		new Selector(contenedorOpciones) => [
			bindEnabledToProperty("miAppModel.hayCalificacionSeleccionada")
			bindItemsToProperty("miAppModel.puntuables").adapter = new PropertyAdapter(Puntuable, "nombre")
			bindValueToProperty("miAppModel.calificacionSeleccionada.evaluado")
		]
		...
		new Label(contenedorFechaColumna) => [
			...
			bindValueToProperty("miAppModel.calificacionSeleccionada.fecha").transformer = new DateTransformer => [
		...
		new Label(contenedorFechaColumna) => [
			bindEnabledToProperty("miAppModel.hayCalificacionSeleccionada")
			bindValueToProperty("miAppModel.calificacionSeleccionada.evaluado.nombre") // .transformer = [ fecha | new SimpleDateFormat("dd/MM/YYYY").format(fecha)]
		]
		...
		new TextBox(contenedorOpciones) => [
			bindEnabledToProperty("miAppModel.hayCalificacionSeleccionada")
			bindValueToProperty("miAppModel.calificacionSeleccionada.detalle") // .transformer = [ fecha | new SimpleDateFormat("dd/MM/YYYY").format(fecha)]
			height = 50
		]
	...
	override void tabla(Panel panel) {
		new Table<Calificacion>(panel, typeof(Calificacion)) => [
			...
			value <=> "miAppModel.calificacionSeleccionada"
